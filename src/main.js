...
const I18N = {
  en: { title:"UP Red Light Areas", sub:"District & area-centroid mapping · information only", sat:"Satellite", street:"Street", lang:"EN", full:"Full", story:"Story", list:"List", search:"Search district or hotel…", fAll:"All", fVer:"Verified", fLow:"Low", legV:"Verified", legTI:"TI district", hint:"Tap a dot to explore", dTitle:"Select a district", dSub:"Tap any red dot or story card.", dContent:"Tap a red dot to explore. Satellite hybrid with labels is default. Map is locked to Uttar Pradesh only.", about:"About this map", aboutBody:`<b>Methodology</b><br/>Compiled from NGO reports (Guria, Freedom Firm), court records, UPSACS TI program data, academic studies and news raids.<br/><b>Coverage:</b> <span id="about-counts"></span><br/><b>Precision:</b> district / area centroids only — never venue-level.<br/><b>Updated:</b> Aug 2026.<br/><i>For information and research only. Report corrections via <a href="https://github.com/HYPERSAHIL/redlight/issues" target="_blank" style="color:#fca5a5">GitHub issues</a>.</i>`, src:"Open source", sv:"Street View", copy:"Copy link", close:"Close", tiType:"District with TI program", relHigh:"High", relMed:"Medium", relLow:"Low", badge:"Verified", introTitle:"Explore<br>the map", introOk:"OK", introStepsM:["Pinch to zoom the map","Tap a red dot for its story","Open List for all 75 districts"], introStepsD:["Scroll to zoom · drag to pan","Click a red dot for its story","Search or open List for districts"], legL:"Low evidence", stats:"{s} sites · {d} districts · {t} TI", sortAZ:"Sorted A–Z", sortRel:"Sorted by reliability" },
  hi: { title:"यूपी रेड लाइट एरिया", sub:"ज़िला व क्षेत्र-केंद्र मानचित्र · केवल जानकारी", sat:"उपग्रह", street:"सड़क", lang:"हि", full:"पूरा", story:"कहानी", list:"सूची", search:"ज़िला या होटल खोजें…", fAll:"सभी", fVer:"प्रमाणित", fLow:"कम", legV:"प्रमाणित", legTI:"टीआई ज़िला", hint:"देखने हेतु बिंदु दबाएँ", dTitle:"कोई ज़िला चुनें", dSub:"कोई लाल बिंदु या कार्ड चुनें।", dContent:"लाल बिंदु दबाएँ। डिफ़ॉल्ट उपग्रह + लेबल। मानचित्र केवल यूपी तक सीमित।", about:"इस मानचित्र के बारे में", aboutBody:`<b>कार्यप्रणाली</b><br/>NGO रिपोर्ट (गुड़िया, फ्रीडम फर्म), न्यायालय रिकॉर्ड, UPSACS TI कार्यक्रम डेटा, शैक्षणिक अध्ययन और समाचार छापों से संकलित।<br/><b>कवरेज:</b> <span id="about-counts"></span><br/><b>सटीकता:</b> केवल ज़िला / क्षेत्र-केंद्र — कभी स्थल-स्तर नहीं।<br/><b>अद्यतन:</b> अगस्त 2026।<br/><i>केवल जानकारी व शोध हेतु। सुधार हेतु <a href="https://github.com/HYPERSAHIL/redlight/issues" target="_blank" style="color:#fca5a5">GitHub issues</a> पर बताएं।</i>`, src:"स्रोत खोलें", sv:"स्ट्रीट व्यू", copy:"लिंक कॉपी", close:"बंद", tiType:"टीआई कार्यक्रम वाला ज़िला", relHigh:"उच्च", relMed:"मध्यम", relLow:"कम", badge:"सत्यापित", introTitle:"मानचित्र<br>देखें", introOk:"ठीक है", introStepsM:["ज़ूम के लिए पिंच करें","कहानी हेतु लाल बिंदु दबाएँ","सभी 75 ज़िलों हेतु सूची खोलें"], introStepsD:["ज़ूम हेतु स्क्रॉल · घुमाने हेतु ड्रैग","कहानी हेतु लाल बिंदु दबाएँ","खोजें या सूची खोलें"], legL:"कम साक्ष्य", stats:"{s} स्थल · {d} ज़िले · {t} टीआई", sortAZ:"क्रम A–Z", sortRel:"विश्वसनीयता क्रम" }
};
const T = () => I18N[lang];
...
let sortMode = LS.get('redlight_sort', 'rel');
function syncSortUI(){
  document.getElementById('search-clear').classList.toggle('on', sortMode === 'az');
}
document.getElementById('search-clear').addEventListener('click', ()=>{
  sortMode = sortMode === 'az' ? 'rel' : 'az';
  LS.set('redlight_sort', sortMode);
  syncSortUI(); renderResults();
  toast(sortMode === 'az' ? T().sortAZ : T().sortRel);
});

let statSites = 0, statDist = 0, statTI = 0;
function updateStats(){
  const el = document.getElementById('stats');
  const txt = T().stats.replace('{s}', statSites).replace('{d}', statDist).replace('{t}', statTI);
  if(el) el.textContent = txt;
  const ac = document.getElementById('about-counts');
...
function renderResults(){
  ...
  const sorted = [...matches].sort((a,b)=>{
    if(sortMode === 'az') return distName(a.properties.district).localeCompare(distName(b.properties.district));
    return (rank[a.properties.reliability] ?? 9) - (rank[b.properties.reliability] ?? 9);
  });
  sorted.slice(0,40).forEach((f,n)=>{
...
function applyLang(){
  ...
  document.querySelector('.about-line').innerHTML = t.aboutBody;
